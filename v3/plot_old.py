'''
march 2025: legend doesn't show when setting hue and plt.legend()
'''
...
class Plot:
    ...
    def ts_plotly(self, df, error_bar=False, agg='mean', alpha=1, threshold=''):
        ...
        for i, (ssp, dd) in enumerate(df.groupby('ssp')):
            ...
            fig.add_trace(go.Scatter(x=dd.date, y=dd[f'{agg}{self.var}'], name=ssp, mode='lines', 
                                     line=dict(color=self.colors[i], width=0.8), legendgroup=ssp))
        fig.update_layout(title=dict(text=f'{self.title} {threshold}',font=dict(size=14)),
                          xaxis_title='Year', yaxis_title=self.var_title,
                          width=1100, height=300, margin=dict(l=20, r=20, t=30, b=20),
    ...
    def severity_plotly(self, df, error_bar=False, agg='mean', alpha=1):
        ...
        fig.update_layout(title=dict(text=f'Severity Comparison: {self.title}',font=dict(size=14)),
                          width=1100, height=300*len(ssps),
    ...
    def variable_comp(self, df, metrics:dict, agg='mean', alpha=1):
        ...
        for i, ssp in enumerate(ssps, 1):
            ...
            for v, (var, t) in enumerate(metrics.items()):
                fig.add_trace(go.Scatter(
                    ...
                    opacity=alpha, line=dict(color=self.colors[v], width=0.8), mode='lines'), row=i, col=1)

        fig.update_layout( 
            title=dict(
            text=f'Variable Comparison: {self.center} - {agg.title()} Across CMIP6 Models {self.month_title}', 
                font=dict(size=14)), width=1100, height=300*len(ssps), paper_bgcolor='white', 
            plot_bgcolor='white', legend_title='Variables', margin=dict(l=50, r=20, t=80, b=50)
        )
        fig.update_annotations(font_size=13)
        for i in range(1, len(ssps)+1):
            fig.update_xaxes(showgrid=True, gridcolor='lightgrey', gridwidth=0.1,  row=i, col=1,
                         title_text='Year' if i == len(ssps) else '')
            fig.update_yaxes(showgrid=True, gridcolor='lightgrey', gridwidth=0.1,  row=i, col=1,
                         title_text='Value' if i == (len(ssps)//2)+1 else '')
        fig.show()
    ...
    # def all(self, df:pd.DataFrame(), t:str, error_bar:bool, plotly_:bool, agg:str='mean', alpha:float=1):
    ...
    #         dd = df[df.threshold == t].drop(columns='threshold')
    #         if len(dd)==0:
    #             raise KeyError(f'Invalid threshold')
    #         self.dist_sns(dd, agg, alpha, t)
    #         severity_plot(df, error_bar, agg, alpha)
