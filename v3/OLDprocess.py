...
class Compound:
    ...
    def process_spi(self):
        ...
        for ssp_name, df in spi_dfs.items():
            ...
            spi_dfs[ssp_name] = pd.concat([SPI().calculate(df, self.base_years, self.spi_years,
    ...
    def process_rzsm(self):
        ...
        hist_df = self.filter_dates(df, self.base_years, False)
    ...
    def process_compound(self, dfs, threshold, thres_df:pd.DataFrame()=None):
        ...
        if len(self.inputs) > 1:
            ...
            compound = compound.add_suffix('_compound')
        return compound
    ...
    def main(self):
        ...
        if self.event in ['CWHE','CDHE']:
            self.spi_years = ((datetime.date(self.base_years[0], 1, 1) - self.delta).year,
                              self.ssp_years[1])
            ...
            groups = {'daily':  pd.concat([spi, spi.mean(axis=1).rename(f'mean_spi'), 
                                           tm,  tm.mean(axis=1).rename(f'mean_tasmax')], axis=1).dropna(),
            ...
            rzsm, thres_rzsm = self.process_rzsm() # ssp dailies, hist threshold
            ...
            groups = {'daily': pd.concat([rzsm, rzsm.mean(axis=1).rename(f'mean_rzsm'),
                                          pr,   pr.mean(axis=1).rename(f'mean_pr')], axis=1).dropna(),
                      'pr': self.group_data(pr.filter(regex='_pr$'), '_pr$').mean().reset_index(),
                      'rzsm': self.group_data(rzsm, '_rzsm$').mean().reset_index(),
                      'thres_rzsm': thres_rzsm
                      }
        elif self.event in ['CWHE2','CDHE2']:
            rzsm, thres_rzsm = self.process_rzsm() # ssp dailies, hist threshold
            tm = self.filter_dates(self.process_tasmax(), self.ssp_years, year_month=False)
            dfs = pd.concat([rzsm, tm], axis=1).dropna()
            groups = {'daily': pd.concat([rzsm, rzsm.mean(axis=1).rename(f'mean_rzsm'),
                                          tm,  tm.mean(axis=1).rename(f'mean_tasmax')], axis=1).dropna(),
                      'tasmax': self.group_data(tm, '_tasmax$').mean().reset_index(),
                      'rzsm': self.group_data(rzsm, '_rzsm$').mean().reset_index(),
            ...
            var = self.inputs[0][:-1]
            dfs = {'rzsm': self.process_rzsm()
            }[var]
            if len(dfs) == 2:
                dfs, thres_rzsm = dfs
            
            groups = {'daily': dfs.dropna(),
                      var: self.group_data(dfs, f'_{var}$').mean().reset_index(),
                      }
            if thres_rzsm is not None:
                groups['thres_rzsm'] = thres_rzsm
        ...
        for threshold in self.thresholds:
            ...
            compounds.append(self.process_compound(dfs, threshold, thres_rzsm))
        ...
        var_aggs = pd.concat(aggs[1], axis=1).reset_index() if aggs[1] else None
