...
def main():
    ...
    for file_name in os.listdir(input_path):
        ...
        center_dfs[center] = pd.merge(center_dfs.setdefault(center, pd.DataFrame(index=pd.DatetimeIndex([]))), 
                                      df, left_index=True, right_index=True, how='outer')
    
    full_date_range = pd.date_range(start='1950-01-01', end='2099-12-31', freq='D')
    for center, df in center_dfs.items():
        df = df.replace(-9999, np.nan)
        df = df.reindex(full_date_range).ffill().bfill()
        
        df.to_csv(os.path.join(output_path, f'{center}_rzsm_ssp245_daily.csv'))
        # Monthly Averages
        monthly_avg = df.resample('ME').mean()
        monthly_avg.index += pd.offsets.MonthEnd(0)
        monthly_avg.to_csv(os.path.join(output_path, f'{center}_rzsm_ssp245_monthly_avg.csv'))
