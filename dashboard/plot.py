...
pio.templates[pio.templates.default].update({
    'layout': {
        ...
        'xaxis': {'showgrid': True, 'gridcolor': 'lightgrey', 'gridwidth': 0.1,
                  'title': {'text': '', 'font': {'size': 11}}
                 },
        'yaxis': {'showgrid': True, 'gridcolor': 'lightgrey', 'gridwidth': 0.1,
                  'title': {'text': '', 'font': {'size': 11}}
                 },
...
class Plot:
    ...
    def distribution(self, df, agg='mean', threshold=''):
        ...
        fig.update_layout(title=f'{self.title} {threshold}', showlegend=True, 
                          ...
                          legend_traceorder='normal', autosize=False,width=800, height=250)
        return fig
    ...
    def time_series(self, df, error_bar=False, agg='mean',threshold=''):
        ...
        for i, (ssp, dd) in enumerate(df.groupby('ssp')):
            ...
            fig.add_trace(go.Scatter(x=dd.date, y=dd[f'{agg}{self.var}'], name=ssp, mode='lines', 
                                     line=dict(color=self.colors[i]), legendgroup=ssp))
        fig.update_layout(title=f'{self.title} {threshold}', autosize=False, width=800, height=300,
                          xaxis_title='Year', yaxis_title=self.var_title.split('(')[0])
        return fig
        ...
        for i, ssp in enumerate(ssps, start=1):
            for j, t in enumerate(thres):
                ...
                fig.add_trace(go.Scatter(x=dd.date, y=dd[f'{agg}{self.var}'], name=t,
                    line=dict(color=self.colors[j % len(self.colors)]), mode='lines',
                    opacity=alpha, legendgroup=t, showlegend=(i==1)), row=i, col=1)

        fig.update_layout(title=dict(text=f'Severity Comparison: {self.title}'),
                          legend_title='Thresholds', autosize=False, width=900, height=250*len(ssps),)
    ...
    def metric_comp(self, df, metrics:dict, agg='mean', alpha=1):
        ...
        fig.update_layout(autosize=False, width=900, height=250*len(ssps), legend_title='Variables',
